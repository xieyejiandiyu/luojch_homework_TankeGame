...
public class Level {
    ...
    private void level001() {
        levelLuoTanK = new LuoTanK(550, 500, 0, 5, 65);
        for (int i = 0; i < 3; i++) {
            levelEnemyTanKArr.add(new EnemyTanK((i + 1) * 100, 100, 1, 5, 65));
            //启动线程
    ...
    private void level002() {
        ...
        for (int i = 0; i < 4; i++) {
            levelEnemyTanKArr.add(new EnemyTanK(70, (i + 1) * 135, 1, 5, 65));
            //建筑物对象赋给敌方坦克
    ...
    private void level003() {
        levelLuoTanK = new LuoTanK(150, 640, 0, 5, 65);
        for (int i = 0; i < 17; i++) {
        ...
        for (int i = 0; i < 4; i++) {
            levelEnemyTanKArr.add(new EnemyTanK((i + 1) * 150, 300, 1, 5, 65));
            levelEnemyTanKArr.get(i).setBuildArr(levelBuildArr);
            //启动线程
            new Thread(levelEnemyTanKArr.get(i)).start();
        }
